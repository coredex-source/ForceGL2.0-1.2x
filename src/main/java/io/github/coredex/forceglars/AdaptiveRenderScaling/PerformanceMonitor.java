package io.github.coredex.forceglars.AdaptiveRenderScaling;

import io.github.coredex.forceglars.config.ForceGLARSConfig;
import net.minecraft.client.MinecraftClient;

public class PerformanceMonitor {
    public static int FPS_UPDATE_INTERVAL_MS = ForceGLARSConfig.CONFIG.instance().updateInterval;
    public static int FPS_CHECK_INTERVAL_MS = ForceGLARSConfig.CONFIG.instance().checkInterval;
    public static int MIN_FPS_THRESHOLD = ForceGLARSConfig.CONFIG.instance().minFpsThreshold;
    public static int MAX_FPS_THRESHOLD = ForceGLARSConfig.CONFIG.instance().maxFpsThreshold;

    private long lastUpdateTime;
    private long lastCheckTime;
    private int avgFps = 0;
    private int intervalCount = 0;

    public void tick() {
        long currentTime = System.currentTimeMillis();
        if (currentTime - lastCheckTime >= FPS_CHECK_INTERVAL_MS) {
            MinecraftClient client = MinecraftClient.getInstance();
            if (client != null) {
                avgFps = avgFps + client.getCurrentFps();
                intervalCount++;
                if (currentTime - lastUpdateTime >= FPS_UPDATE_INTERVAL_MS){
                    avgFps = Math.floorDiv(avgFps, intervalCount);
                    AdaptiveChunkScaling.adjustRenderDistance(avgFps);
                    avgFps = 0;
                    intervalCount = 0;
                    lastUpdateTime = currentTime;
                }
            }
            lastCheckTime = currentTime;
        }
    }

    public int getCurrentFps() {
        MinecraftClient client = MinecraftClient.getInstance();
        return client.getCurrentFps();
    }
}